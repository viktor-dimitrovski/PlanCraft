// frontend/plancraft-ui/src/pages/App.jsx
...
export default function App(){
  ...
  // Global event: other panels can request grid refresh
  useEffect(() => {
    const f = () => refreshGrid()
    window.addEventListener('plancraft:refresh', f)
    return () => window.removeEventListener('plancraft:refresh', f)
    // rebind when these change so the handler uses current range/scenario
  }, [range.from, range.to, scenarioId])

  // Move/Copy/Resize tasks
  const onTaskMove = (task, newPersonId, weekIndex, newSpan, copy=false, resized=false) => {
    ...
    moveTask({
      taskId: task.id,
      newStartDate: new Date(w.start).toISOString(),
      newPrimaryPersonId: newPersonId,
      newDurationDays: newSpan ? newSpan * 7 : undefined,
      copy
    }).then(() => refreshGrid())
  }
  ...
  const onCreateTask = async (personId, weekIndex, title) => {
    ...
    const tRes = await apiCreateTask({
      ...
      title,
      estimatedDays: 5,
      startDate: new Date(w.start).toISOString(),
      durationDays: 7,
      status: 1,
      requiredSkills: []
    })
  ...
  // Utilization indicators (simple thresholds)
  useEffect(() => {
    ...
    <div className="app">
      <div className="topbar">
        ...
        <div className="toolbar">
          ...
          <select
            value={scenarioId || ''}
            onChange={e => setScenario(e.target.value ? parseInt(e.target.value, 10) : null)}
          >
      ...
      <div className="container">
        <aside className="sidebar">
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            <input
              type="date"
              value={weeks?.[0]?.start?.slice?.(0, 10)}
              onChange={e => setRange(r => ({ ...r, from: new Date(e.target.value) }))}
            />
            <input
              type="date"
              value={weeks?.[colCount - 1]?.end?.slice?.(0, 10)}
              onChange={e => setRange(r => ({ ...r, to: new Date(e.target.value) }))}
            />
          </div>
          <p style={{ fontSize: 12, color: '#64748b' }}>
            Tips: Drag to move; drag the right handle to resize. Hold Ctrl/Alt while dropping to copy.
          </p>
        </aside>
        ...
        <aside className="rightbar">
          ...
          <ul>
            {milestones.map((m, i) => (
              <li key={i}>
                <span className="legendDot" style={{ background: m.color }}></span>
                {' '}{m.projectName}: <b>{m.name}</b> — {new Date(m.date).toLocaleDateString()}
              </li>
            ))}
          </ul>
          <hr />
          ...
          <ul>{utilPanel.over.map((x, i) => (
            <li key={'o' + i}>{x.person} — {new Date(x.week).toLocaleDateString()} — {x.util}%</li>
          ))}</ul>
          <div className="badge">Underutilized ({utilPanel.under.length})</div>
          <ul>{utilPanel.under.map((x, i) => (
            <li key={'u' + i}>{x.person} — {new Date(x.week).toLocaleDateString()} — {x.util}%</li>
          ))}</ul>
          <hr />
          <h3>Compare</h3>
          {compare.length === 0
            ...
            : <ul>{compare.map((c, i) => (
                <li key={i}>
                  {projects.find(p => p.id === c.projectId)?.name}: Slip {Math.round(c.slipDays)} days
